...
import { X, Calendar, Clock, MapPin, MessageSquare, Send, CheckCircle2, ShieldCheck, AlertCircle, Lock, Shield, Info } from 'lucide-react';
          ...
          <form onSubmit={handleSubmit} className="p-5 space-y-4 overflow-y-auto flex-1">
            ...
            <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 flex items-center justify-between">
              ...
              <button
                ...
                className="flex items-center gap-1.5 text-xs text-emerald-600 dark:text-emerald-400 font-semibold hover:underline cursor-pointer"
              >
            ...
            <div className="p-3 rounded-2xl bg-emerald-50/80 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800/80 text-xs text-emerald-900 dark:text-emerald-200 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Lock className="w-4 h-4 text-emerald-600 shrink-0" />
                <span className="font-medium">
                  <strong>PIN Antigolpe:</strong> Você receberá um código de 4 dígitos para liberação presencial.
                </span>
              </div>
              <button
                type="button"
                onClick={() => setIsSecurityModalOpen(true)}
                className="text-[11px] font-bold text-emerald-700 dark:text-emerald-400 underline shrink-0 cursor-pointer ml-2"
              >
                Como funciona
              </button>
            </div>
            ...
            <div className="pt-2">
              <button
                type="submit"
                disabled={isLoading}
                className="w-full py-3.5 bg-gradient-to-r from-orange-500 to-amber-600 hover:from-orange-600 hover:to-amber-700 text-white font-bold rounded-2xl shadow-lg shadow-orange-500/20 transition flex items-center justify-center gap-2 disabled:opacity-50 cursor-pointer"
              >
                {isLoading ? (
                  ...
                  <>
                    <Send size={18} />
                    <span>Confirmar e Enviar Chamado Seguro</span>
                  </>
                )}
              </button>
            </div>
